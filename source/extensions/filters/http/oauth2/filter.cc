...
bool OAuth2Filter::canSkipOAuth(Http::RequestHeaderMap& headers) const {
  ...
  if (validator_->isValid()) {
    ...
    if (config_->forwardBearerToken() && config_->forwardBearerTokenType() == TokenType::AccessToken && !validator_->token().empty()) {
      setBearerToken(headers, validator_->token());
    }
    if (config_->forwardBearerToken() && config_->forwardBearerTokenType() == TokenType::IdToken && !validator_->idToken().empty()) {
      setBearerToken(headers, validator_->idToken());
    }
    if (config_->forwardBearerToken() && config_->forwardBearerTokenType() == TokenType::RefreshToken && !validator_->refreshToken().empty()) {
...
void OAuth2Filter::finishRefreshAccessTokenFlow() {
  ...
  if (config_->forwardBearerToken() && config_->forwardBearerTokenType() == TokenType::AccessToken  && !access_token_.empty()) {
    setBearerToken(*request_headers_, access_token_);
  }
  if (config_->forwardBearerToken() && config_->forwardBearerTokenType() == TokenType::IdToken  && !id_token_.empty()) {
    setBearerToken(*request_headers_, id_token_);
  }
  if (config_->forwardBearerToken() && config_->forwardBearerTokenType() == TokenType::RefreshToken  && !refresh_token_.empty()) {
