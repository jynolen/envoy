...
config:
  ...
  EXPECT_EQ(proto_config.config().forward_bearer_token_type(), ::envoy::extensions::filters::http::oauth2::v3::OAuth2Config_TokenType::OAuth2Config_TokenType_ACCESS_TOKEN);
}
...
config:
  ...
  const auto result = factory.createFilterFactoryFromProto(proto_config, yaml, context);
  EXPECT_TRUE(result.ok()) << result.status().message();
  EXPECT_EQ(proto_config.config().forward_bearer_token_type(), ::envoy::extensions::filters::http::oauth2::v3::OAuth2Config_TokenType::OAuth2Config_TokenType_REFRESH_TOKEN);
}
...
config:
  ...
  const auto result = factory.createFilterFactoryFromProto(proto_config, yaml, context);
  EXPECT_TRUE(result.ok()) << result.status().message();
  EXPECT_EQ(proto_config.config().forward_bearer_token_type(), ::envoy::extensions::filters::http::oauth2::v3::OAuth2Config_TokenType::OAuth2Config_TokenType_ID_TOKEN);
}

TEST(ConfigTest, ForwardBearerTypeIsInvalid) {
  const std::string yaml = R"EOF(
config:
  forward_bearer_token: true
  forward_bearer_token_type: foobarToken
  authorization_endpoint: https://oauth.com/oauth/authorize/
  redirect_uri: "%REQ(x-forwarded-proto)%://%REQ(:authority)%/callback"
  credentials:
    client_id: "secret"
  ...
  const auto result = factory.createFilterFactoryFromProto(proto_config, yaml, context);
  EXPECT_FALSE(result.ok()) << result.status().message();
}
} // namespace Oauth2
} // namespace HttpFilters
} // namespace Extensions
